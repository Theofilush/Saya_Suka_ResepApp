...
import id.web.skytacco.sysuka.adapter.MinumanAdapter;
...
public class MinumanFragment extends Fragment {
    ...
    List<MinumanItem> arrayMinumanItem;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        arrayMinumanItem = new ArrayList<>();
    ...
    public void clearData() {
        int size = this.arrayMinumanItem.size();
        if (size > 0) {
            for (int i = 0; i < size; i++) {
                this.arrayMinumanItem.remove(0);
            }

            mMinumanAdapter.notifyItemRangeRemoved(0, size);
        }
    }

    public void setAdapterToRecyclerView() {
        mMinumanAdapter = new MinumanAdapter(getActivity(), arrayMinumanItem);
        recyclerView.setAdapter(mMinumanAdapter);
    }
    ...
    private class MyTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            if (null == result || result.length() == 0) {
                ...
                for (int j = 0; j < arrayMinumanItem.size(); j++) {
                    MinumanItem mMinumanItem = arrayMinumanItem.get(j);

                    array_cat_id.add(mMinumanItem.getCatId());
                    array_cat_name.add(mMinumanItem.getCategoryName());
                    array_cid.add(String.valueOf(mMinumanItem.getCId()));
                    array_image.add(String.valueOf(mMinumanItem.getNewsImage()));
                    array_title.add(String.valueOf(mMinumanItem.getNewsHeading()));
                    array_desc.add(String.valueOf(mMinumanItem.getNewsDescription()));
                    array_date.add(String.valueOf(mMinumanItem.getNewsDate()));
